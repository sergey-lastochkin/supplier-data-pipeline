...
| Источник | Базовый URL | Формат | Лицензия |
| --- | --- | --- | --- |
...
каталоги с одинаковой API-структурой. Для чтения авторизация не требуется,
...
следующий запрос минимум на 7 секунд. При 429/5xx безопасные запросы на чтение
повторяются с ограниченной растущей задержкой.
...
гарантированы поставщиком API, поэтому проверка схемы и пропуск неполных записей
